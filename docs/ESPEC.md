...
Versão 1.1 · 02/09/2026 · Arquiteto. Consolida o BRIEF.md e os 4 relatórios de pesquisa
...
**Mudanças da 1.1 (alinham o contrato ao que a build entrega — leia antes de auditar):**
o global do core é **`CL`** (não `CA`); as chaves de armazenamento usam o prefixo **`clinicar.v1.`**;
os arquivos do app ficam na **raiz do repositório**, porque é a raiz que o GitHub Pages publica —
onde a 1.0 apontava para uma subpasta, leia "os arquivos do app"; a identidade visual é a da §7.1
revisada (verde Clinicar), e não mais o azul genérico da 1.0.
...
2. Nenhum nome de concorrente no produto. Nenhuma menção ao ecossistema anterior: a string proibida não aparece nos arquivos do app, em `functions/`, `firestore.rules`, `CNAME` nem em meta tags.
3. Zero `alert()`, `confirm()`, `prompt()`. Só `CL.ui.modal / confirm / toast / drawer`.
4. Ninguém cria projeto Firebase, mexe em regras publicadas, IAM ou credenciais. `config.js` fica vazio.
5. Nunca gravar o estado inteiro por cima (last-write-wins). Só `CL.upsert / CL.remove / CL.setCfg`, que passam pelo merge item a item do `backend.js`.
6. Nenhum `_mt*.js` no app. Nenhuma conta ou Firebase do ecossistema anterior.
...
- Estado em memória: `CL.state = { <coleção>: [], cfg: {...}, _tomb: {id: tsMs} }`.
- Todo item de coleção tem `id` (string, sem ponto — `CL.uid()` = `Date.now().toString(36) + '-' + 6 chars base36`), `createdAt` (ms) e `updatedAt` (ms). `updatedAt` é carimbado SEMPRE por `CL.upsert`; construtores não carimbam à mão.
- Exclusão = `CL.remove(col, id)` → grava `_tomb[id] = Date.now()` e filtra. Coleções clínicas (evolucoes, receitas, documentos, exames, auditoria) **não têm exclusão na interface**; só inativação/versão.
...
- `cfg` é um objeto único com `updatedAt`; no merge vence o `updatedAt` maior (nunca é reescrito parcialmente a partir de um form — `CL.setCfg(patch)` mescla sobre o existente).
- Logo e outros blobs ficam FORA do estado principal (`clinicar.v1.logo` é cache do navegador; a fonte, em firebase, é o doc `meta/logo` da conta). Por estarem fora do `state`, entram no envelope da exportação — senão sumiriam no "Sair" e não voltariam pelo backup.
...
{ id, data:'YYYY-MM-DD', hora:'HH:MM', dur:30, profId, pacId, procId,
  ...
  createdAt, updatedAt }
```
Consulta nunca é apagada pela interface: cancelar muda status. `CL.remove` em consultas só existe para desfazer criação acidental nos primeiros 60 s (toast Desfazer).
...
users/{uid}/meta/logo  ({ dataUrl, updatedAt })  users/{uid}/meta/info  ({ versao, ultimoSync })
```
...
Antes de tudo o importador cria (se não existir) **um profissional padrão** a partir de `cfg` antigo e **os 5 procedimentos** de semente; todo item importado recebe `profId` desse profissional. Convênios são criados a partir do texto `convenio` normalizado (`CL.util.norm`), sem duplicar.
...
| `cfg.logo` (dataURL) | `clinicar.v1.logo` (cache) / `meta/logo` (fonte, na conta) | fora do estado; entra no envelope do backup |
...
Os arquivos do app ficam na **raiz do repositório** — é essa pasta que o GitHub Pages publica
(`medtechbr.github.io/clinicar/`). Não existe pasta intermediária.
...
├── core.js                CL: estado, storage, roteador, modal/toast, utilitários
...
├── manifest.webmanifest, icone-192.png, icone-512.png, .nojekyll
├── firestore.rules
├── functions/index.js
├── CNAME                  (modelo em docs/CNAME.exemplo)
└── docs/ ESPEC.md · PENDENTE.md · DOMINIO.md · EXPORTACAO-ANTIGO.md · RELATORIO.md · fixtures/
```

Ordem de carga no `index.html` (scripts clássicos, `defer`, nesta ordem): `config.js → core.js → backend.js → login.js → pacientes.js → agenda.js → atendimento.js → financeiro.js → painel.js → configuracoes.js → importar.js`. Cada módulo expõe UM global (`CL`, `Backend`, `Login`, `Agenda`, `Pacientes`, `Atendimento`, `Financeiro`, `Painel`, `Config`, `Importar`). Nenhum `import`/`export` ES (sem bundler; scripts clássicos para `file://` e GitHub Pages).
...
Contrato de módulo de tela (todos os que têm rota): `X.mount(el, params)` desenha dentro de `el` (limpa antes), `X.unmount()` remove listeners/timers. O roteador chama `unmount` do anterior e `mount` do novo. Módulos não tocam no DOM fora do `el` recebido, exceto via `CL.ui.*`.
...
Responsabilidade: casca única. `<head>` com `<title>Clinicar</title>`, meta viewport, `theme-color` (`#0B6B4F`, igual ao `--acento`), manifest, Inter + a família de display (Google Fonts, com fallback `system-ui`), Tabler Icons webfont (cdn.jsdelivr `@tabler/icons-webfont`), `styles.css`. `<body>`: honeypot anti-autofill logo após `<body>` (`<input name="username" … tabindex="-1" aria-hidden>` + `password`, ambos fora da tela), `#app` com:
...
Depende de: todos os scripts. Sem JS inline além de `CL.boot()` no final.
...
### 4.4 `core.js` (dono: casca) — global `CL`

Depende de: `Backend` (chamado em `boot`), `Login` (gate). Todo mundo depende de `CL`.

Estado e persistência
- `CL.state` — objeto vivo (seção 3). Leitura direta permitida; **escrita só pelas funções abaixo**.
- `CL.ready: Promise<void>` — resolve após `boot` carregar o estado e a sessão.
- `CL.col(nome) → Array` — a coleção (cria vazia se faltar).
- `CL.get(nome, id) → obj|undefined`.
- `CL.upsert(nome, obj) → obj` — atribui `id` se faltar, `createdAt` se faltar, `updatedAt = Date.now()`; substitui/insere; marca sujo; `CL.persist()`; emite `'change'` com `{col:nome, id, obj}`. Devolve o objeto guardado.
- `CL.patch(nome, id, campos) → obj` — `Object.assign` + mesma trilha de `upsert`.
- `CL.remove(nome, id) → boolean` — lápide + filtra + persist + `'change'`.
- `CL.setCfg(patchProfundo) → cfg` — mescla recursivamente sobre `state.cfg`, carimba `updatedAt`, persist, emite `'cfg'`.
- `CL.persist() → Promise<void>` — debounce 300 ms → `Backend.save(state, {sujos})`; em falha: `CL.ui.toast(msg, {kind:'erro', fixo:true, action:{label:'Exportar agora', fn}})`. Nunca engole erro.
- `CL.on(evento, fn) → unsub`, `CL.emit(evento, dado)`. Eventos: `'change'`, `'cfg'`, `'route'`, `'session'`, `'sync'` (`{estado:'ok'|'offline'|'erro'|'local', em}`), `'consulta:status'` (`{id, de, para}`), `'privacidade'` (`bool`).
- `CL.uid() → string`.
- `CL.lote(fn) → Promise<void>` — executa `fn` com o debounce de `persist` suspenso e faz UM `Backend.save` ao final (importação, seed, restauração).
- `CL.seed()` — na primeira execução cria procedimentos, convênio Particular, usuário admin "Administração" e modelos de WhatsApp padrão; `cfg.seed=true`.

Sessão e permissões
- `CL.session` — `{ usuarioId, nome, perfil, profId, privacidade:false }` ou `null`.
- `CL.session.set(usuario)`, `CL.session.clear()`, `CL.can(acao) → boolean` com ações `'clinico'` (abrir evolução/receita/documento: profissional e admin), `'financeiro'` (recepção, admin), `'config'` (admin), `'agenda'` (todos). Recepção não passa em `'clinico'` — a UI esconde e o roteador redireciona com toast "Seu perfil não abre o prontuário".
- `CL.audit(acao, alvo, alvoId, extra)` — insere em `auditoria` (com teto).
...
- `CL.route.register(vista, modulo)` — `modulo` tem `mount(el, params)` / `unmount()`. `params = { seg:[...], q:{...} }`.
- `CL.route.go(hash, {replace:false})`, `CL.route.current → {vista, seg, q, hash}`, `CL.route.voltar()`.
...
- `CL.ui.modal({ titulo, corpo: string|Element, botoes:[{ rotulo, tipo:'primario'|'perigo'|'neutro', acao(ctx), fecha:true }], largo:false, aoFechar }) → { el, fechar() }` — remove qualquer modal anterior antes de abrir; Esc fecha; foco preso; `aria-modal`; foco volta ao disparador.
- `CL.ui.confirmar({ titulo, texto, ok:'Confirmar', okTipo:'primario'|'perigo', cancelar:'Cancelar' }) → Promise<boolean>`.
- `CL.ui.pedirTexto({ titulo, rotulo, valor:'', placeholder }) → Promise<string|null>`.
- `CL.ui.toast(msg, { kind:'ok'|'erro'|'aviso'|'info', ms:4000, fixo:false, action:{ rotulo, fn } }) → { fechar() }` — no máximo 3 visíveis; com `action` dura 6 s.
- `CL.ui.drawer({ titulo, corpo, rodape, largura:'md'|'lg', aoFechar }) → { el, fechar() }` — painel à direita (desktop) / folha inferior (≤ 767 px); Esc fecha; só um por vez.
- `CL.ui.vazio(el, { icone, titulo, texto, acao:{ rotulo, fn } })`, `CL.ui.carregando(el, texto)` (esqueleto sem pulo), `CL.ui.erro(el, { texto, acao })`.
- `CL.ui.menu(ancoraEl, [{ rotulo, icone, tipo, fn }])` — menu "…" posicionado.

Formatação e utilitários
- `CL.fmt.data(ymd) → 'dd/mm/aaaa'`, `CL.fmt.dataExtenso(ymd)`, `CL.fmt.diaSemana(ymd, curto)`, `CL.fmt.hora(hhmm)`, `CL.fmt.dinheiro(cent) → 'R$ 1.234,56'`, `CL.fmt.idade(nasc) → '34 a'|''`, `CL.fmt.fone(digits)`, `CL.fmt.cpf(digits)`, `CL.fmt.relativo(ms)`.
- `CL.util.hoje() → 'YYYY-MM-DD'`, `ymd(date)`, `addDias(ymd, n)`, `min('HH:MM') → int`, `hhmm(int)`, `somaMin('HH:MM', n)`, `norm(str)` (minúsculas sem acento), `esc(str)` (HTML), `iniciais(nome)`, `debounce(fn, ms)`, `digits(str)`, `centavos('12,50') → 1250`, `sha256(str) → Promise<hex>`, `baixar(nomeArquivo, texto, mime)`, `semAutofill(inputEl)` (limpa valor com "@" no foco e ignora no filtro).
- `CL.STATUS` — `{ agendado:{ rotulo:'Agendado', icone:'ti-calendar', classe:'st-agendado', terminal:false }, … }` para os 9 status. `CL.FLUXO = ['agendado','confirmado','chegou','em_atendimento','finalizado']`. `CL.proximoStatus(s) → s|null`.
- `CL.print.documento({ titulo, corpoHtml, paciente, profissional, tipoDoc:'receita'|'atestado'|'exames'|'documento'|'evolucao'|'agenda', vias:1|2, rotulosVias:['1ª via — farmácia','2ª via — paciente'], validade:'', mostrarCid:false }) → Promise<void>` — monta A4 no `#print-raiz` (iframe oculto, `srcdoc`, sem `window.open`, sem `document.write`), cabeçalho legal (nome + conselho/UF, RQE, clínica, endereço, telefone/e-mail), linha do paciente (nome · CPF · nascimento), data/hora de emissão, corpo, assinatura + carimbo, rodapé fixo "Documento gerado eletronicamente. Válido quando impresso e assinado pelo profissional ou assinado digitalmente com certificado ICP-Brasil." + `cfg.clinica.rodape`; imprime `vias` páginas rotuladas; registra `CL.audit`. CPF vazio → modal "Sem CPF na ficha — imprimir assim mesmo?".
- `CL.keys.register(vista, { 't': fn, 'n': fn, … })` — atalhos ativos só na vista atual e fora de inputs.
- `CL.pref.get(chave, padrao)` / `CL.pref.set(chave, valor)` — preferências de interface em `clinicar.v1.pref` (densidade, profissionais visíveis, última visão).
- `CL.boot() → Promise<void>` — `Backend.init(window.CLINICAR_CONFIG)` → `Backend.load()` → `state` → `seed` → `Backend.subscribe` → `Login.gate()` → `route.start()`.

### 4.5 `backend.js` (dono: integracao) — global `Backend`

Interface única; dois adaptadores internos escolhidos em `init`. Depende de: `CL.util`, `CL.ui.toast` (só para avisos de cota), nunca de módulos de tela.
...
- `Backend.backups() → [{ chave, em, contagens:{ pacientes, consultas, … } }]`, `Backend.restaurar(chave) → Promise<state>`, `Backend.chavesRuins() → [...]`, `Backend.exportar() → string` (JSON `{ app:'clinicar', versao:1, exportadoEm, logo, state }` — a `logo` entra no envelope porque vive fora do `state`; sem ela o backup não reproduz os documentos impressos), `Backend.limparRuins()`.
- `Backend.logo.get() → dataUrl|''` (síncrono: a impressão lê no meio da montagem), `Backend.logo.set(dataUrl) → Promise<boolean>`, `Backend.logo.pendente() → boolean`. **A logo é da CONTA, não do navegador:** `clinicar.v1.logo` é só cache; no adaptador firebase a fonte é `users/{uid}/meta/logo` `{ dataUrl, updatedAt }`, lida em `load` e por `onSnapshot`, gravada em `set`. `updatedAt` maior vence; navegador que nunca teve logo nunca sobe vazio por cima da conta. Antes de `auth.sair` limpar o cache, uma logo pendente é enviada (senão "Sair" a perderia); na troca de conta a logo do dono anterior é descartada com o resto do cache e nunca sobe para a conta que entrou.
- `Backend.status() → { modo, online, ultimoSaveOk, ultimoSync, pendentes }`.

Blindagem do adaptador local (obrigatória):
1. Chaves: `clinicar.v1.state`, `clinicar.v1.bk.1|2|3`, `clinicar.v1.ruim.<ts>`, `clinicar.v1.meta` `{ ultimoSaveOk, ultimoExport, contagemUltimoSave, sessaoSalvou, uid, logo:{ updatedAt, sincronizadaEm } }`, `clinicar.v1.logo`, `clinicar.v1.sessao`, `clinicar.v1.pref`, `clinicar.v1.rascunhos`. `auth.sair` e a troca de conta apagam todas as `clinicar.v1.*` menos `clinicar.v1.pref` (preferência de interface não é dado de paciente).
...
- `Login.sair()` — `CL.audit('logout')`, limpa sessão, `Backend.auth.sair()` se firebase, volta ao gate.
- `Login.trocarUsuario()` — mesma lista sem deslogar do Firebase.

### 4.7 `agenda.js` (dono: agenda) — global `Agenda`

Depende de: `CL`, `Pacientes.buscar / rapido / faltas / abrirFicha`, `Financeiro.lancarDaConsulta` (via evento), `Atendimento.iniciar`.
...
- `Agenda.salvar(dados, { forcarEncaixe:false }) → { ok:true, consulta } | { ok:false, conflitos:[...] }` — valida, grava via `CL.upsert('consultas')`, historico `'criada'`/`'editada'`.
...
- `Agenda.imprimirDia(data, profId)` → `CL.print.documento({ tipoDoc:'agenda' })`.

### 4.8 `pacientes.js` (dono: agenda) — global `Pacientes`

Depende de: `CL`, `Atendimento` (abre editores das abas clínicas), `Financeiro.extrato` (aba Financeiro), `Agenda.abrirNova` (botão Agendar).
...
Depende de: `CL` (upsert, print, ui, audit, can), `Backend.ai`, `Agenda.mudarStatus`, `Pacientes.faltas` (não), `Financeiro.baixa` (via Agenda).
...
- `Atendimento.evolucao` — `{ abrir({ pacId, consultaId, id }) → editor, salvar(dados) → evolucao (edição empurra a versão anterior para `versoes`), rascunho:{ get(chave), set(chave, texto), limpar(chave) } (autosave a cada 2 s em `clinicar.v1.rascunhos` (chave própria: é texto clínico, então sai no "Sair" junto com o resto — o `pref` fica)), ditar:{ iniciar(onTexto), parar(), disponivel() }, gravar:{ iniciar() → Promise, parar() → Promise<{ base64, mime, seg }>, emAndamento() }, estruturar({ tipoAtend, formato, rascunho, pacId }) → Promise<string> (monta o prompt escriba com `encounterCtx`), transcrever({ base64, mime, tipoAtend, formato, pacId }) → Promise<string>, resumoPaciente(pacId) → Promise<string> }`. Formatos `REC_FMT` reaproveitados do sistema antigo (regras de anamnese incluídas). Consentimento verbal para gravar exibido antes de iniciar (checkbox lembrado por sessão).
- `Atendimento.receita` — `{ abrir({ pacId, id, itens? }), salvarLote(dados) → [receita] (uma receita por tipo — nunca mistura simples/antimicrobiano/controle na mesma folha; por isso é lote e não `salvar`), ver(id), imprimir(id|rascunho), buscarMed(q) → [{ n, p, c }] (MED_DB reaproveitado), classificar(item), separar(itens) → { simples:[], antimicrobiano:[], controle:[] }, texto(dados), print(dados), tipos, modelos:{ listar(), salvar(nome, dados), aplicar(id) } }`. Impressão: antimicrobiano 2 vias + "Validade: 10 dias"; controle 2 vias + "Validade: 30 dias" + quantidade por extenso obrigatória (bloqueia com modal se faltar); listas A/B → aviso "use a Notificação de Receita oficial" e não imprime.
...
Depende de: `CL`, `Pacientes.buscar` (lançamento avulso).
...
Depende de: `CL`, `Agenda.mudarStatus / abrirConsulta / lembretes`, `Financeiro.resumo`, `Pacientes.abrirFicha`, `Atendimento.iniciar`, `Backend.status`.
Rota: `#/painel`.
...
Depende de: `CL`, `Backend` (backups, exportar, logo, status), `Importar.mount`, `Login.trocarUsuario`.
...
- `Config.exportarTudo()` — `Backend.exportar()` → `CL.util.baixar('clinicar-backup-AAAA-MM-DD.json')`; `meta.ultimoExport`; `CL.audit('dados.exportar')`.
...
Depende de: `CL`, `Backend.exportar` (backup automático antes de aplicar).
...
- `Importar.aplicar(parcial, { modo:'mesclar'|'substituir' }) → Promise<{ contagens }>` — mesclar = `Backend.merge` item a item (importado só vence se `updatedAt` maior); substituir = exporta backup, confirma com REMOVER, troca as coleções importadas. Sempre via `CL.upsert` em lote (`CL.persist` uma vez ao fim — expor `CL.lote(fn)` no core que suspende o debounce).
- `Importar.relatorio(contagens, avisos) → html` — só números.

### 4.14 Fora dos módulos do app
...
**Extrato** (`#/financeiro/extrato`): filtros (período com atalhos Hoje/Semana/Mês, profissional, forma, convênio, status), tabela paginada (50), totais por forma e por profissional, exportar CSV (`CL.util.baixar`).
...
Ambiente de teste: `python3 -m http.server 88xx` na raiz do repositório, aba anônima, dados fictícios. "Grep" = busca no código do app (os `.js` e o `index.html` da raiz).

| # | Critério | Como testar |
|---|---|---|
| CA-01 | Zero `alert(`, `confirm(`, `prompt(` nos `.js` e no `index.html`. | `grep -nE "\b(alert|confirm|prompt)\(" *.js index.html` devolve 0 linhas. |
| CA-02 | A string proibida (nome do ecossistema anterior) e nomes de concorrentes não aparecem nos arquivos do app, em `functions/`, `firestore.rules`, `CNAME`, `manifest`. | `grep -ri "medtech" *.js *.html *.css *.webmanifest functions firestore.rules docs/CNAME.exemplo` devolve 0; grep dos 20 nomes de concorrentes dos relatórios devolve 0. |
| CA-03 | Nenhum `_mt*.js` no app nem referência a ele. | `ls | grep _mt` vazio; `grep -n "_mt" *.js index.html` vazio. |
...
| CA-11 | Zero emoji como ícone: nenhum caractere das faixas emoji fora de conteúdo de usuário. | `grep -P "[\x{1F300}-\x{1FAFF}\x{2600}-\x{27BF}]" *.js index.html` devolve 0. |
...
| CA-14 | Cadastro rápido de dentro do drawer cria paciente com nome + telefone e a consulta fica vinculada (`pacId`). | Criar "Paciente Teste" → salvar → `CL.get('consultas', id).pacId` existe. |
...
| CA-34 | Dinheiro em centavos e formatado pt-BR em toda a interface (`R$ 1.234,56`); datas `dd/mm/aaaa`. | Inspecionar 5 telas; `grep -n "toFixed(2)" *.js` deve ser 0 fora de `CL.fmt`. |
...
| CA-36 | Trava do 1º save: com dados gravados, forçar `CL.state.pacientes=[]; CL.state.consultas=[]; CL.persist()` numa sessão nova NÃO grava e abre a recuperação. | Conferir que `clinicar.v1.state` mantém as contagens. |
...
| CA-41 | Sincronização firebase nunca grava o estado inteiro: `grep -n "setDoc" backend.js` só em `meta/*` e por item; nenhum `setDoc` com `state` completo; nenhum fallback após transação falha. | Revisão de código + teste com `config.js` fictício apontando para projeto inexistente → toast de erro, dados locais intactos. |
...
| CA-51 | A logo sobrevive ao "Sair" e segue a conta: em modo firebase, `Backend.logo.set` grava `users/{uid}/meta/logo`; `auth.sair` envia a pendente antes de limpar o cache; ao entrar de novo (ou em outro aparelho) a logo volta. | Subir logo → Sair → entrar: a logo reaparece nos documentos. `grep -n "meta', 'logo'" backend.js` mostra leitura, snapshot e gravação. |
| CA-52 | A exportação leva a logo: `Backend.exportar()` devolve `logo` no envelope e reimportar o arquivo devolve a logo (sem inventar uma quando o arquivo não tem). | Exportar tudo → conferir `logo` no JSON → remover a logo → importar o arquivo → logo de volta. |
...
Identidade Clinicar: **um acento só** (verde profundo), papel quente, tinta com viés verde. **Um único `:root`**
no topo de `styles.css` — é proibido redefinir tokens numa "camada de tema" mais abaixo (a primeira build fazia
isso e o contrato passou a discordar do produto). Media query só ajusta `--alvo`, `--hora-px` e `--drawer-w`.

```
--acento:#0B6B4F; --acento-hover:#085440; --acento-fraco:#E2F0E9; --acento-texto:#0A5A42; --acento-claro:#5FD3A6;
--agora:#C8442A; --agora-fraco:#FCE7E0; --agora-texto:#A03318;   (o "agora": em atendimento, linha da hora)
--tinta:#17211C; --tinta-2:#3E4944; --tinta-3:#5C665F;
--papel:#FBFAF7; --painel:#F0EEE7; --linha:#E4E1D7; --linha-forte:#CFCBBD; --branco:#FFFFFF;
--ok:#0B5B39; --ok-fraco:#DCEFE4; --aviso:#7A4A00; --aviso-fraco:#FFEBCC; --erro:#98252F; --erro-fraco:#FADDE0;
--sombra-1:0 1px 1px rgba(23,33,28,.05); --sombra-2:0 4px 14px rgba(23,33,28,.08); --sombra-3:0 14px 40px rgba(23,33,28,.14);
--raio-1:4px; --raio-2:8px; --raio-3:12px;
...
--hora-px:60px; (48 compacto / 80 confortável — atributo data-densidade no body; 72 em ≤ 768)
--nav-w:64px; --topo-h:52px; --drawer-w:420px; (380 em ≤ 768) --alvo:40px; (44 em ≤ 768)
```
Cores semânticas só para status/feedback. Sem gradientes. `body { background:var(--papel); color:var(--tinta); font:var(--t-14)/1.45 var(--fonte); font-feature-settings:'tnum' 1 }` para horários e dinheiro.

**Tipografia:** `--fonte` (Inter) em TODO texto de dado, formulário, tabela e agenda; `--display` só em `h1/h2/h3`,
`.marca`, `.ag-cabeca h2` e `.vista-titulo`. Duas famílias com papéis separados — nunca display em conteúdo.

**Cor da marca fora do CSS:** `<meta name="theme-color">`, `theme_color` do manifest e os ícones PWA usam `#0B6B4F`;
`background_color` do manifest usa `#FBFAF7` (o `--papel`). Os três andam junto com o `--acento`.
...
| status | classe | fundo | texto | contraste | ícone |
|---|---|---|---|---|---|
| Agendado | `.st-agendado` | `#E8EAED` | `#333B45` | 9,4:1 | `ti-calendar` |
| Confirmado | `.st-confirmado` | `#DEEAFB` | `#14459E` | 7,3:1 | `ti-check` |
| Chegou | `.st-chegou` | `#FFEBCC` | `#7A4A00` | 6,4:1 | `ti-door-enter` |
| Em atendimento | `.st-em_atendimento` | `--agora-fraco` `#FCE7E0` | `--agora-texto` `#A03318` | 5,9:1 | `ti-stethoscope` |
| Finalizado | `.st-finalizado` | `#DCEFE4` | `#0B5B39` | 6,8:1 | `ti-circle-check` |
| Faltou | `.st-faltou` | `#FADDE0` | `#98252F` | 6,2:1 | `ti-user-off` |
| Cancelado | `.st-cancelado` | `#EDEBE4` | `#5C665F` (riscado) | 5,0:1 | `ti-x` |
| Cancelado tarde | `.st-cancelado_tarde` | `#FADDE0` | `#98252F` (riscado) | 6,2:1 | `ti-clock-x` |
| Cancelado pela clínica | `.st-cancelado_clinica` | `#EDEBE4` | `#5C665F` (riscado) | 5,0:1 | `ti-building-off` |
